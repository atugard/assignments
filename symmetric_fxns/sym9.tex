...
\begin{proof}
  ...
  Let $\lambda \in Par$ be maximal (under partition inclusion $\subseteq$) such that $c_\lambda \neq 0$, and let $\mu \in Par$.

  If there's some part of $\lambda$ that's not a part of $\mu$, say $\lambda_i$, then
  \begin{align*}
    \cfrac{\partial}{\partial p_{\lambda_i}}p_\mu = 0.
  \end{align*}
  Otherwise, $\lambda$ and $\mu$ have all the same parts. If $\lambda$ has a greater multiplicity of some part $\lambda_j$, then 
  \begin{align*}
    (\cfrac{\partial}{\partial p_{\lambda_j}})^{m_{\lambda_j}(\lambda)}p_\mu = 0.
  \end{align*}
  Otherwise, $m_{\lambda_j}(\lambda) \leq m_{\lambda_j} (\mu)$ for each $1 \leq j \leq \ell(\lambda)$; but then $\lambda \subseteq \mu$. If $\lambda \subsetneq \mu$, we get $c_\mu = 0$. 
  ...
  \begin{align*}
    ...
    &=
    c_\lambda\prod\limits_{\substack{1\leq i \leq \ell(\lambda) \\ \lambda_i \text{ distinct}}} \lambda_i^{m_{\lambda_i}(\lambda)} m_{\lambda_i}(\lambda)! \\
    &=
    c_\lambda\prod\limits_{i \geq 1} i^{m_{i}(\lambda)} m_{i}(\lambda)!\\
    &=
    c_\lambda z_\lambda
  \end{align*}
