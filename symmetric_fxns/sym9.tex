...
\end{exercise}
\begin{proof}
  If $\lambda, \mu$ are not comparable under partition inclusion, then there's some $\lambda_i$ that does not appear as a part in $\mu$, and so 
  \begin{align*}
    \cfrac{\partial}{\partial p_{\lambda_i}}p_\mu = 0.
  \end{align*}
  By our choice of $\lambda$ we have that for $\lambda \subsetneq \mu$, $c_\mu = 0$. Also, $\prod\limits_{i=1}^{\ell(\lambda)} \cfrac{\partial}{\partial p_{\lambda_i}}p_\mu = 0$ if $\mu \subsetneq \lambda$. We use all of these facts in the following:
  \end{align*}
  \begin{align*}
    p_\lambda^-f
    &=
