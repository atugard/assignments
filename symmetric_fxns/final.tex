...
\begin{proof}
  ...
  \begin{align*}
    e_r(\underbrace{1, \dots, 1}_{\text{n entries}}) = \sum\limits_{T \in RowStrict((k), n)} 1 =
    \begin{pmatrix}
      n \\
      k
    \end{pmatrix},
  \end{align*}
  since a strictly increasing filled row of $k$ boxes with entries from $1$ to $n$ corresponds to a choice of $k$ distinct numbers from $1$ to $n$ up to permutation, and there are
  $\begin{pmatrix}
      n \\
      k
  \end{pmatrix}$
